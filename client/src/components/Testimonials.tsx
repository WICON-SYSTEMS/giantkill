      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        <StaggerContainer className="grid md:grid-cols-2 gap-8">
          {testimonials.map((testimonial, index) => (
            <HoverCard key={index} scale={1.02} rotate={1}>
              <Card className="hover-elevate" data-testid={`card-testimonial-${index}`}>
                <CardContent className="p-8">
                  <Quote className="h-10 w-10 text-chart-2 mb-4 group-hover:rotate-12 transition-transform duration-200" />
                  <p className="text-lg mb-6 italic">"{testimonial.quote}"</p>
                  <div className="flex items-center gap-4">
                    <Avatar className="h-16 w-16 group-hover:scale-110 transition-transform duration-200">
